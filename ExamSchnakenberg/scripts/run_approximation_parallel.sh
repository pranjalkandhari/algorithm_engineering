...
REPORT_FILE=../output/approximation_parallel_report.csv

OUT_FOLDER=../output/Track1/approximation_parallel/
...
for FILENAME in ../input/Track1/*; do
    ...
    COMMAND="timeout ${TIMEOUT_LIMIT} ${EXECUTABLE} -gr ${FILENAME} -t ${NUM_THREADS} -o ${OUTFILE}"
    OUT=$($COMMAND)
